...
def detectTube(frameUndistorted, a):
    ...
    # Detect the skeleton line of the tube
    ...
    for contour in contours:
        ...
        #cv2.drawContours(frameUndistorted, [contour*2], -1, (0, 255, 0), 3)
        for point in contour:
            #cv2.circle(frameUndistorted, point[0], 2, (255, 0, 0), -1)
            #point_cloud = np.append(point_cloud, [point[0]], axis=0)
            point_cloud_new.append(point[0])
            
    #print(len(point_cloud))
    
    # Downsample the point cloud
    pts = np.array(point_cloud_new, dtype=np.float32)
    ...
    ========clustering method ========
    # define the model
    model = MiniBatchKMeans(n_clusters=len(point_cloud)//4, n_init='auto')
    ...
    for cluster in clusters:
        ...
        cv2.circle(frameUndistorted, (int(new_pc[i, 0]), int(new_pc[i, 1])), 2, (255, 0, 0), -1)
    ...
    new_frame = frameUndistorted.copy()    
    new_pc = sortPoints(new_pc, new_frame)
    cv2.imshow('point cloud', new_frame)
    cv2.waitKey(1)
    new_pc = np.append(new_pc, [new_pc[0, :]], axis=0) 
    
    '''
    #plt.scatter(new_pc[:, 0], new_pc[:, 1])
    plt.plot(new_pc[:, 0], new_pc[:, 1])
    #fig = plt.show()
    plt.pause(0.005)
    plt.ioff()
    plt.cla()
    ...
    # Curve fitting
    harmNum = 20    
    gs = cf.generateDetectedGs(new_pc, harmNum)
    coff = cf.curveFitting(gs, new_pc.reshape(2*len(new_pc), 1), harmNum)
        
    gs_new = cf.generateGs(0, 0.001, 1, 0, 0, harmNum)
    pt_fitted = gs_new * coff
    points = np.array([pt_fitted[0::2], pt_fitted[1::2]]).T
    cv2.polylines(frameUndistorted, np.int32([points]), isClosed=False, color=(0, 0, 255), thickness=3)


def sortPoints(points, frameUndistorted):
    point_cloud = np.empty((0, 2), int)
    point = points[-1, :]
    ...
    while len(points) > 0:
        ...
        cv2.circle(frameUndistorted, (int(points[min_index, 0]), int(points[min_index, 1])), 2, (255, 0, 0), -1)
        points = np.delete(points, min_index, axis=0)
        point = point_cloud[-1, :]
...
if __name__ == '__main__':
    ...
    #image = cv2.imread('/home/binzhang/zbin13/codingsomethingcool/epuck_ws/src/epuck_driver_cpp_wifi/detect2.jpg')
